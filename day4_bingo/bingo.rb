module SquidBingo
  class Game < Struct.new(:cards, :draws, keyword_init: true)
    class Result < Struct.new(:bingo_card, :final_draw); end
  ...
  class Card
    class Place < Struct.new(:value, :marked)
      def initialize(value, marked = false)
        super(value, marked)
      end
    ...
    def mark(number)
      position_index[number]&.tap do |row_i, col_i|
        matrix[row_i][col_i].marked = true
      end
    end
    ...
    def sum_marked
      matrix.flatten.select(&:marked).map(&:to_i).sum
    end

    def sum_unmarked
      matrix.flatten.reject(&:marked).map(&:to_i).sum
    end

    def to_s
      matrix.map { |line| line.map(&:to_s).join(" ") }.join("\n")
    end

    private

    def position_index
      @position_index ||= matrix.each_with_index.flat_map do |line, row_i|
        line.each_with_index.map { |place, col_i| [place.value, [row_i, col_i]] }
      end.to_h
    end

    def matrix
      @matrix ||= @lines.map { |line| line.split(" ").map(&Place.method(:new)) }
    end
  ...
  class GameEngine
    def self.build_game(file_input:)
      lines = file_input.dup
      draws = lines.shift.split(",")
      cards = lines.each_slice(6)
        .map { |lines| lines.drop(1) }
        .map(&Card.method(:new))

      Game.new(cards: cards, draws: draws)
    end
  end
end
